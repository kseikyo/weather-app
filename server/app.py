...
API_KEY = dotenvConfig.get("WEATHER_KEY") or ""
...
def search_city(city):
    ...
    if response.get('cod') != 200:
        return make_response(jsonify(error="Sorry, We couldn't find the specified city."), response.get('cod'))
    ...
    if current_temperature:
        ...
        return make_response(jsonify(error=f"Sorry, We couldn't find any city called {city.tile()}."), response.get('cod'))
